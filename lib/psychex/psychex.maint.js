...
class pRectangle extends Primitive{
    ...
    draw(){
        ...
        if (this.isHovered) {
            scale(1.05); // Slightly enlarge the button when hovered
        }

        // Style the button based on hover and click state
        if (this.isClicked) {
            fill(this.kwargs.clickedFill || 'darkgray');
            scale(0.98);
        } else if (this.isHovered) {
            fill(this.kwargs.hoverFill || 'lightgray');
            scale(1.02)
        } else {
            fill(this.kwargs.objFill || '#f0f0f0');
        }
        rect(0, 0, dims.x, dims.y, 4);
...
class Utils{
     ...
     * @returns {boolean} true if touch device (mobile or tablet), else false
     */
     ...
     * @returns {boolean} return true if device is in landscape, false if not
     */
    static isLandscape(){
...
class GridWorld extends Primitive {
    ...
    handleMovement(mode, preMovementCallback = () => {}, postMovementCallback = () => {}){
        ...
        if (!["arrows", "wasd", "click", "buttons"].includes(mode)){throw new Error(`Mode type ${mode} not recognised. Must be one of: arrows, wasd, click.`)};
        ...
        if (mode == "arrows"){
            ...
            // Look for a buttons object and attach the callback to each button
            if (this.buttons == undefined){
                console.log("No object named this.buttons found - to automatically attach the callback to buttons, name the object this.buttons. This function will now return the callback for you to manually attach.")
                return callback;
            } else {
                Object.values(this.buttons).forEach(b => {b.onClick = callback});
            }
        }
    }
